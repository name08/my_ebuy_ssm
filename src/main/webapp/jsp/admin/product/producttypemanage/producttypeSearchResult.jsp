    ...
    <div class="tools">
        ...
        <form action="<%=basePath%>/admin/product/producttypemanage/productTypeSearchResult" method="get">
            <ul style="float:right;">
                ...
                <li style="float: left;margin-left: 15px"><input name="returnProductTypeManage" type="button" class="btn" value="返回产品分类管理"/></li>
            </ul>
        </form>

    </div>
        <c:choose>
            <c:when test="${productTypes.totalDataCount!=0&&productTypes.totalDataCount>0}">
    ...
    <div class="pagin">
        ...
        <ul class="paginList">
            ...
            <c:if test="${productTypes.pageCount eq 1}">
                ...
                <li class="paginItem"><a href="<%=basePath%>/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=1">首页</a></li>
                <li class="paginItem"><a href="<%=basePath%>/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=1">上一页</a></li>
            ...
            <li class="paginItem"><a href="<%=basePath%>/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${page.pageCount-1}">上一页</a></li>

            </c:if>
            <c:if test="${productTypes.totalPageCount<5}">
                <c:forEach begin="1" end="${productTypes.totalPageCount}" varStatus="index">

                    <li class="paginItem"><a id="${index.index }" href="<%=basePath%>/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${index.index}"><font>${index.index}</font></a></li>
            ...
            <c:if test="${productTypes.totalPageCount>5}">
                <c:if test="${productTypes.pageCount>=4&&productTypes.pageCount<=productTypes.totalPageCount-2}">
                    <c:forEach begin="${productTypes.pageCount-2}" end="${productTypes.pageCount+2}" varStatus="index">
                        <li class="paginItem"> <a id="${index.index }" href="${pageContext.request.contextPath}/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${index.index}"><font>${index.index}</font></a></li>
                    </c:forEach>
                </c:if>
            </c:if>
            ...
            <li class="paginItem"> <a href="${pageContext.request.contextPath}/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${productTypes.totalPageCount}">下一页</a></li>
            <li class="paginItem"> <a href="${pageContext.request.contextPath}/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${productTypes.totalPageCount}">尾页</a></li>
            </c:if>

            <c:if test="${productTypes.pageCount < productTypes.totalPageCount}">
            <li class="paginItem"><a href="${pageContext.request.contextPath}/admin/product/producttypemanage/productTypeSearchResult?keyWord=${productTypes.keyWord}&pageCount=${productTypes.pageCount+1}">下一页</a></li>
        ...
        <c:otherwise>
            <center><br>很遗憾，没有查询到带有 <font style="color: red;">  ${productTypes.keyWord} </font> 的产品分类，换一个关键词再试试吧。</center>
        </c:otherwise>
