...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $("input[name='returnCustomersManage']").click(function() {
    ...
    <div class="tools">
        ...
        <form action="<%=basePath%>admin/CustomerManage/CustomerSearchResult" method="get">
            <ul style="float:right;">
                ...
                <li style="float: left;margin-left: 15px"><input name="returnCustomersManage" type="button" class="btn" value="返回顾客管理"/></li>
            </ul>
        </form>
    </div>

    <c:choose>
    <c:when test="${customers.totalDataCount!=0&&customers.totalDataCount>0}">
    <table class="tablelist">
        ...
        <c:forEach var="customer" items="${requestScope.customers.list}" varStatus="index">
            <tr>
                ...
                <td><a href="<%=basePath%>admin/CustomerManage/toUpdateCustomer?id=${customer.id}" class="tablelink">编辑顾客信息</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" name="deleteCustomer" alt="${customer.id}" class="tablelink">删除顾客信息</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<%=basePath%>admin/CustomerManage/doResetCustomerPassword?id=${customer.id}" name="ResetCustomerPassword" class="tablelink">重置顾客密码</a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    ...
    <div class="pagin">
        ...
        <ul class="paginList">
            ...
            <c:if test="${customers.pageCount eq 1}">
                ...
                <li class="paginItem"><a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=1">首页</a></li>
                <li class="paginItem"><a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=1">上一页</a></li>
            ...
            <c:if test="${customers.pageCount > 1}">
                ...
                <li class="paginItem"><a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=${page.pageCount-1}">上一页</a></li>

            </c:if>
            <c:if test="${customers.totalPageCount<5}">
                <c:forEach begin="1" end="${customers.totalPageCount}" varStatus="index">

                    <li class="paginItem"><a id="${index.index }" href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=${index.index}"><font>${index.index}</font></a></li>
            ...
            <c:if test="${page.pageCount eq page.totalPageCount}">
                ...
                <li class="paginItem"> <a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=${customers.totalPageCount}">下一页</a></li>
                <li class="paginItem"> <a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=${customers.totalPageCount}">尾页</a></li>
            </c:if>

            <c:if test="${customers.pageCount < customers.totalPageCount}">
                <li class="paginItem"><a href="<%=basePath%>admin/CustomerManage/queryCustomersPage?c_sex=${customers.c_sex}&keyWord=${customers.keyWord}&pageCount=${customers.pageCount+1}">下一页</a></li>
        ...
        <c:otherwise>
            <center><br>很遗憾，没有查询到相关 <font style="color: red;">${customers.keyWord} <c:choose><c:when test="${customers.c_sex==-1}"></c:when><c:when test="${customers.c_sex==0}">男性</c:when><c:when test="${customers.c_sex==1}">女性</c:when></c:choose>  </font> 客户的信息，换一个关键词或性别再试试吧。</center>
        </c:otherwise>
